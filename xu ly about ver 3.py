...
# 📌 Define prediction thresholds for each entity type
# These are the minimum GraphSAGE prediction scores to accept a relationship
...
# 📌 Process About nodes and extract entities using only GraphSAGE predictions
print("\n🔍 Processing About nodes and extracting entities using pure GraphSAGE predictions...")
...
for _, row in employees_with_about.iterrows():
  ...
  entity_types = ['Skill', 'Technology', 'Experience_Level', 'Project', 'Soft_Skill']
  ...
  for entity_type, entities in extracted_entities.items():
    ...
    for entity in entities:
      ...
      temp_entity_embedding = create_temp_entity_embedding(entity, entity_type, node_embeddings)
      ...
      edge_embedding = torch.cat([employee_embedding, temp_entity_embedding], dim=0).unsqueeze(0)

      # Use model to predict link probability
      with torch.no_grad():
        prediction_score_tensor = torch.sigmoid(model.mlp(edge_embedding).squeeze())
        prediction_score = prediction_score_tensor.item()

      # Get threshold for this entity type
      threshold = PREDICTION_THRESHOLDS.get(entity_type, 0.6)

      # Only keep relationships with prediction score above threshold
      if prediction_score > threshold:
        ...
        print(f"  🔄 {entity_type}: {entity[:50]}{'...' if len(entity) > 50 else ''} [Score: {prediction_score:.2f}]")
  ...
  for rel in new_relationships:
    ...
    cypher_query = f"""
            ...
            SET r.prediction_score = $prediction_score,
    ...
    try:
      ...
      print(
        f"  ✅ Added: {rel['employee_name']} -[:{rel['relationship_type']} ({rel['prediction_score']:.2f})]-> {rel['entity_type']}:{rel['entity'][:30]}..."
      )
    except Exception as e:
